...
class Star(pygame.sprite.Sprite):
    def __init__(self, groups):
        super().__init__(groups)
        self.image = pygame.image.load(join('image', 'star_small.png')).convert_alpha()
...
async def main():
    global running, game_over, alien_alive, level, ice_monsters_spawned, alien
    ...
    while running:
        ...
        if game_over:
            ...
            pygame.display.update()
            # Yield to browser so UI doesn’t freeze under pygbag
            await asyncio.sleep(0)
            continue
        ...
        for event in pygame.event.get():
            ...
            if event.type == meteor_event:
                ...
                image_path = 'image/star_small.png'  # Replace with actual icons if you have them
                # NOTE: your original call signature was off; PowerUp expects (groups, pos, surf, kind)
                PowerUp((all_sprites, powerup_group), (random.randint(25, Window_Width-25), -50), powerup_surf, kind)
            elif event.type == pygame.USEREVENT + 1:
                player.speed = 300
            if event.type == powerup_event:
                x = random.randint(0, Window_Width - 50)
                kind = random.choice(['speed', 'shield', 'diagonal_arrows', 'more_damage'])
                PowerUp((all_sprites, powerup_sprites, powerup_group), (x, -50), powerup_surf, kind=kind)
            if event.type == pygame.KEYDOWN:
        ...
        # Boss/enemy spawns and state logic (unchanged)
        # (kept exactly as in your original loop)
        # ------------------------------------------------------------------
        # Print debug
        ...
        if alien_alive:
            ...
            if level == 3 and len(enemies) == 0:
                for _ in range(5):
                    health = randint(50, max(150, alien.max_health - 10))
                    speed = randint(2, 5)
                    enemy = Enemy(join('image', 'enemy.png'), health, speed, bullet_group)
                    enemies.add(enemy); all_sprites.add(enemy)
                    enemy2 = Enemy2(join('image', 'enemy2.png'), health, speed, bullet_group)
                ...
                for _ in range(1):
                    ...
                    enemies.add(enemy3)
                    # all_sprites.add(enemy3)  # you had this commented
        ...
        # Power-up collisions (unchanged except we avoid blocking delays later)
        ...
        if powerup_collision:
            ...
            if await ask_math_question():
                ...
                pygame.display.update()
                # pygame.time.delay(1000)  # <- blocking; replace later if needed
            else:
                pass
        ...
        if player.shield_active:
            pygame.draw.circle(display_surface, (0, 0, 255), player.rect.center, player.rect.width, 5)

        # Your “player death / respawn” logic (unchanged, but note: ask_math_question is blocking)
        if not player.alive():
            if await ask_math_question():
                # NOTE: your original referenced `answer` here, but it’s undefined.
                # If ask_math_question() returned True, just revive directly.
                player.alive = True
            else:
                player.alive = True
                player.rect.center = (Window_Width / 2, Window_Height - 100)
            ...
            if not ice_monsters_spawned:
                for _ in range(3):
                    ...
                    enemies.add(ice_enemy); all_sprites.add(ice_enemy)
                ice_monsters_spawned = True

        if level == 8 and len(enemies) == 0 and not ice_monsters_spawned:
            for _ in range(4):
        ...
        if level == 9 and len(enemies) == 0 and not ice_monsters_spawned:
            for _ in range(5):
        ...
        if level == 10 and len(enemies) == 0:
            ...
            for _ in range(1):
                health = 850
                speed = randint(1, 3)
                # You created `alien` with enemy.png and then added `enemy` (undefined). Fixed:
                alien = Alien(join('image', 'enemy.png'), health, speed, bullet_group)
                enemies.add(alien); all_sprites.add(alien)

        if level == 11:
            volcano_background = pygame.image.load(join('image', 'volcano.png'))
            volcano_background = pygame.transform.smoothscale(volcano_background, (Window_Width, Window_Height))
            display_surface.blit(volcano_background, (0, 0))

        pygame.display.update()

        # Let the browser / pygbag event loop breathe
        await asyncio.sleep(0)

    # end while
    # no hard pygame.quit() here; pygbag handles teardown
